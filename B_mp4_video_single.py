...
#seed_number = 718
#static_seed_for_interpolation = 781
#smoothness = 2.0

def main(seed_number, static_seed_for_interpolation,smoothness):
    ...
    _G, _D, Gs = pickle.load(open("results/00005-stylegan2-yoga_dataset-2gpu-config-f/network-snapshot-001564.pkl", "rb"))
    ...
    random_seed = seed_number
    mp4_file = 'results/interpolate_%s.mp4' % random_seed
    mp4_codec = 'libx264'
    mp4_bitrate = '5M'
    ...
    num_frames = int(np.rint(duration_sec * mp4_fps))
    random_seed = seed_number
    random_state = np.random.RandomState(static_seed_for_interpolation)
    #random_state = np.random.RandomState(random_seed)
